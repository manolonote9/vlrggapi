...
Provides async scraper functions:
  ...
  - vlr_team_history: categorized roster with transaction enrichment (fork)
  - vlr_team_roster: grouped roster by category (upstream)
  - vlr_team_upcoming: upcoming matches from team page sidebar (fork)
  - vlr_team_schedule: upcoming matches from team page sidebar (upstream)
"""
import logging
import re
...
# ---------------------------------------------------------------------------
# Team history — categorized roster with transaction enrichment (fork endpoint)
# ---------------------------------------------------------------------------


@handle_scraper_errors
async def vlr_team_history(team_id: str) -> dict:
    """Scrape team roster history with categorized player groups.

    Custom fork endpoint that groups players into active/staff/former/benched
    with transaction date enrichment.
    """
    ...
    return await cache_manager.get_or_create_async(
        CACHE_TTL_TEAM_HISTORY, build, *cache_key
    )
...
# ---------------------------------------------------------------------------
# Team upcoming matches — fork endpoint from team page sidebar
# ---------------------------------------------------------------------------
...
async def vlr_team_upcoming(team_id: str) -> dict:
    ...
    async def build():
        ...
        if status >= 400:
            logger.warning("Non-200 response %d for team upcoming %s", status, team_id)
            raise HTTPException(
                status_code=status,
